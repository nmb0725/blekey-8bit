...
```
      ...
      ▼                 ▼                 ▼
┌──────────┐    ┌──────────────┐   ┌────────────┐
│ macros.py│    │ settings.py  │   │wifi_manager│
│ 事件模型  │    │ 配置加载/保存 │   │  .py       │
│ 预设宏    │    │ settings.json│   │ WiFi 管理  │
│ 自定义宏  │    └──────┬───────┘   └────────────┘
└──────────┘           │
                       ▼                 ┌──────────────┐
                  ┌──────────┐     ┌─────┤web_config.py │
...
多个修饰键通过按位或（OR）组合，例如 `0x01 | 0x02 = 0x03` 表示 Ctrl+Shift。
    ...
    while True:
        ...
        if trigger_held:
            ...
            if trigger_press_start[0] == 0:
                trigger_press_start = [time.ticks_ms()] * len(trigger_indices)
            if time.ticks_diff(time.ticks_ms(), trigger_press_start[0]) >= long_press_ms:
                # 进入配置模式
                await config_mode()
                continue
        else:
...
按顺序执行宏中的事件列表，根据事件类型调用不同的 BLE 发送方法。`cancellable` 参数控制是否检查取消标志——循环调用传 `True`，一键宏调用传 `False`：
...
| `run_events_async(macro, idx, cancellable=False)` | 异步执行宏中的事件序列，cancellable 控制是否响应取消标志 |
...
- **功能**：
  ...
  - 序列化时额外输出 `key`/`modifiers`/`button` 字段，支持单键映射宏在前端可视化编辑
...
- **功能**：
  ...
  - 增/删事件行
  - **↑↓ 移动按钮**：调整事件行执行顺序，首行 ↑ 禁用、末行 ↓ 禁用
  - **单键映射宏加载**：CAPS/CTRL/F12 等单键宏进入编辑器时自动生成 press+release 事件行，不再显示空白
...
2. ESP32 连接预设 WiFi 时 LED 快速闪烁（200ms），连接成功后 LED 熄灭
3. 浏览器访问串口输出的 IP 地址
4. 在配置页面修改按键的宏绑定或引脚号
