...
├── settings.py          # 配置数据模型、JSON 加载/保存、宏注册表
...
- **用途**：配置数据模型和持久化管理
- **功能**：
  - 定义 `KeyMacro`、`KeyEvent`、`MouseEvent` 等核心数据结构
  - 管理 `settings.json` 的加载/保存/合并
  - 维护 30+ 预置宏的注册表
  - 支持自定义宏的 JSON 序列化/反序列化

### 7.3 wifi_manager.py（新增）
